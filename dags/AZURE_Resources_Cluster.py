...
def fetch_from_database(resource_id):
    ...
    # cursor.execute('SELECT "resourcesId" FROM "Request" WHERE id = %s;', (resource_id,))
    # res = cursor.fetchone()
    # if not res:
...
with DAG(
    ...
    # Workflow
    get_resource_id >> get_config_info >> create_tf_dir >> write_tf_files >> terraform_apply >> branch_task >> trigger_aks >> end
